...
def rgb_to_grayscale(observation):
    """Convert an RGB image to grayscale using the luminosity method."""
    grey_observation = cv2.cvtColor(observation, cv2.COLOR_RGB2GRAY)
    return grey_observation

def rescale_image(observation, size):
    """Resize the image to 84x84."""
    resized_observation = cv2.resize(observation, size, interpolation=cv2.INTER_AREA)
