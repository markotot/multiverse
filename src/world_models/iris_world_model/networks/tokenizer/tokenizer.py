...
from src.world_models.iris_world_model.networks.tokenizer.lpips import LPIPS
from src.world_models.iris_world_model.networks.tokenizer.nets import Encoder, Decoder
from src.world_models.iris_world_model.utils import LossWithIntermediateLosses
...
class Tokenizer(nn.Module):
    ...
    def compute_loss(self, dataset: torch.Tensor, **kwargs: Any) -> LossWithIntermediateLosses:
    ...
    def encode(self, x: torch.Tensor, should_preprocess: bool) -> TokenizerEncoderOutput:
        ...
        b, e, h, w = z.shape
        z_flattened = einops.rearrange(z, 'b e h w -> (b h w) e')
        dist_to_embeddings = torch.sum(z_flattened ** 2, dim=1, keepdim=True) + torch.sum(self.embedding.weight**2, dim=1) - 2 * torch.matmul(z_flattened, self.embedding.weight.t())

        tokens = dist_to_embeddings.argmin(dim=-1)
        z_q = einops.rearrange(self.embedding(tokens), '(b h w) e -> b e h w', b=b, e=e, h=h, w=w).contiguous()
